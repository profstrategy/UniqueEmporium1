...
import { Star, User, Mail, MessageSquare, Loader2, CheckCircle2 } from "lucide-react"; // Added User, Mail, MessageSquare, Loader2, CheckCircle2
...
interface ProductReviewsTabProps {
  ...
  productId: string; // Added productId to allow adding new reviews
}
...
const ProductReviewsTab = ({ reviews, productId }: ProductReviewsTabProps) => {
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    const submittedReviewItem = {
      ...
      isVerifiedBuyer: false, // New reviews are not verified by default
      ...newReview,
    };

    setSubmittedReviews((prev) => [submittedReviewItem, ...prev]);
    setNewReview({ rating: 0, title: "", comment: "", author: "", email: "" }); // Clear form
      ...
      <CardContent className="p-6">
        ...
        {allReviews.length > 0 && (
          <motion.div variants={reviewVariants} initial="hidden" animate="visible" className="mb-8 text-center">
            <h3 className="font-poppins text-3xl font-bold text-foreground mb-2">{averageRating} / 5</h3>
        ...
        <motion.div variants={reviewVariants} initial="hidden" animate="visible" className="mb-12">
          <h3 className="font-semibold text-xl text-foreground mb-4">Write a Review</h3>
            ...
            {allReviews.map((review, index) => (
              <motion.div key={review.id} variants={reviewVariants} initial="hidden" animate="visible">
                ...
                <p className="text-sm text-muted-foreground mb-2 flex items-center">
                  By <span className="font-medium text-foreground ml-1">{review.author}</span> on {new Date(review.date).toLocaleDateString()}
                ...
                {index < allReviews.length - 1 && <Separator className="mt-8" />}
              </motion.div>
            ))}
          </div>
        )}
