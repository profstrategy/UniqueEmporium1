...
import { ChevronLeft, ChevronRight, Box } from "lucide-react"; // Changed Cube to Box
...
const ProductImageGallery = ({ images, has3DModel = false }: ProductImageGalleryProps) => {
      ...
      <div className="relative h-[300px] sm:h-[400px] md:h-[500px] bg-muted flex items-center justify-center">
        <AnimatePresence initial={false} mode="wait">
          {is3DView ? (
            <motion.div
              key="3d-model"
              className="absolute inset-0 flex items-center justify-center bg-gray-200 text-muted-foreground"
            ...
            >
              ...
              <Box className="h-24 w-24 text-gray-400" /> {/* Changed Cube to Box */}
              <p className="absolute bottom-4 text-sm">3D Model Viewer (Placeholder)</p>
            </motion.div>
          ) : (
            <motion.div
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center space-x-2 z-10">
          ...
          {has3DModel && (
            ...
            >
              <Box className="h-4 w-4 mr-2" /> {is3DView ? "2D View" : "3D View"} {/* Changed Cube to Box */}
            </Button>
          )}
        </div>
      </div>
